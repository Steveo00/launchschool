# Write a method that takes a string, and then returns a hash that contains 3 entries: 
# - one represents the percentage of characters in the string that are lowercase letters, 
# - one the percentage of characters that are uppercase letters, and 
# - one the percentage of characters that are neither.

# You may assume that the string will always contain at least one character.

def letter_percentages(string)
  results = Hash.new(0)
  
  characters = string.split(//)

  results[:lowercase] = characters.count { |ch| ch =~ /[a-z]/ }
  results[:uppercase] = characters.count { |ch| ch =~ /[A-Z]/ }
  results[:neither]   = characters.count { |ch| ch =~ /[^a-zA-Z]/ }

  results[:lowercase] = results[:lowercase] / string.length.to_f * 100
  results[:uppercase] = results[:uppercase] / string.length.to_f * 100
  results[:neither]  = results[:neither] / string.length.to_f * 100

  results
end

puts letter_percentages('abCdef 123') == { lowercase: 50, uppercase: 10, neither: 40 }
puts letter_percentages('AbCd +Ef') == { lowercase: 37.5, uppercase: 37.5, neither: 25 }
puts letter_percentages('123') == { lowercase: 0, uppercase: 0, neither: 100 }
